      ...
      <div className="space-y-6">
        {records.map((record, index) => (
          <div key={record.id || index} className="relative flex gap-4 pl-10">
            {/* Dot */}
            <div className="absolute left-2.5 top-1 w-3 h-3 rounded-full bg-blue-600 border-2 border-white shadow" />
            ...
            <div className="flex-1 bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
              ...
              <p className="text-sm text-gray-800 font-medium">{record.summary}</p>
              {record.remarks && (
                <p className="text-xs text-gray-500 mt-1 italic">{record.remarks}</p>
              )}
            </div>
          </div>
        ))}
